...
from models import BertCRF, BertForTokenClassification, BertLSTM, BertLSTMCRF
...
def get_model_and_kwargs_for_args(
        args: Arguments,
        training: bool = True,
) -> tuple[Union[type[BertLSTM], type[BertLSTMCRF], type[BertForTokenClassification], type[BertCRF]], dict[str, Any]]:
    ...
    if args.freeze_bert:
        ...
        if args.no_crf:
            return BertLSTM, model_args
        else:
            return BertLSTMCRF, model_args
    else:
        # Possible models: BertForNERClassification or BertCRF
        if args.no_crf:
            return BertForTokenClassification, model_args
        else:
            return BertCRF, model_args
